module.exports = function(config, gulp, taskOpts) {
  var api, extraHelp, less, plumber, q, runTask;
  if (taskOpts == null) {
    taskOpts = {};
  }
  q = require('q');
  ...
  runTask = function(src, dest, base, appOrRb, loc) {
    ...
    gulp.src(src, {
      base: base
    }).pipe(plumber()).pipe(less()).pipe(gulp.dest(dest)).on('end', function() {
  ...
  api = {
    runTask: function(loc) {
      return extraHelp.run.tasks.async(runTask, 'compile', 'less', [loc]);
    }
  };
